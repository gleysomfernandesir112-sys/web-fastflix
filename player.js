document.addEventListener('DOMContentLoaded', () => {
    ...
    window.openTvModal = function() {
        ...
        if (!modal) {
            console.error('Modal de TV não encontrado');
            return;
        }
        ...
        for (let sub in allChannels.tv) {
            if (Array.isArray(allChannels.tv[sub])) {
                tvChannels = tvChannels.concat(allChannels.tv[sub]);
            }
        }
        ...
        function renderChannels(filter = '') {
            ...
            const filteredChannels = tvChannels.filter(channel => channel.title.toLowerCase().includes(lowerFilter));
            ...
            filteredChannels.forEach(channel => {
                ...
                `;
                li.addEventListener('click', () => {
                    const url = 'player-page.html?videoUrl=' + encodeURIComponent(channel.url);
                    if (debounceNavigation(url)) {
                        window.location.href = url;
                    }
                });
        ...
        renderChannels(); // Renderiza a lista inicial
        modal.classList.add('show');
    }
